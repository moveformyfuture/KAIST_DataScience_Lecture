...
class HashTable:
    ...
    def calculateHashFunction(self,strWord):
        ...
        for i in range(len(strWord)):
            hashValue = hashValue + ord(strWord[i]) * ord(strWord[i]) # 문제
        return hashValue % self.capacity # 문제
    ...
    def put(self, strWord, idxWord):
        ...
        if self.size >= self.capacity / 2:
            self.doubleSize()
        hashValue = self.calculateHashFunction(strWord) # 문제
        while True:
            idx = (hashValue + trial + trial*trial ) % self.capacity  # 문제
