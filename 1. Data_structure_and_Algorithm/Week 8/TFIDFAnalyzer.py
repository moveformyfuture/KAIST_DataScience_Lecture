...
class TFIDFAnalyzer:
    ...
    def __init__(self,folder,maxFileNumber):
        dataset = BagOfWordCreator.BagOfWord(folder,maxFileNumber)
    ...
    def calculateTermFrequency(self):
        ...
        for idxWord in range(len(self.words)):
            ...
            for idxBow in range(len(self.bows)):
                if idxWord in self.bows[idxBow]: # 문제
                    cnt = cnt + self.bows[idxBow][idxWord] # 문제
    ...
    def calculateTFIDF(self):
        ...
        for itr in range(len(self.words)):
            ...
            TFIDF.append(termFrequencies[itr] * math.log(self.numDocuments / documentFrequencies[itr])) # 문제
        return retWords, TFIDF

if __name__ == "__main__":
    analyzer = TFIDFAnalyzer('./talk.politics.misc', 10)
