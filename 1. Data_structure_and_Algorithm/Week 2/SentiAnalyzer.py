...
class SentiAnalyzer:
    ...
    def runAnalysis(self, idxReview):
        ...
        if self.dataReviewTesting[idxReview] == 1:
            if probLogPositive > probLogNegative :  # 1번문제
                correct = 1 # 2번문제
            else:
                correct = 0 # 3번문제
        else:
            if probLogPositive > probLogNegative : # 4번문제 
                correct = 0 # 5번문제
            else: 
                correct = 1 # 6번문제
    ...
    def runWholeAnalysis(self):
        ...
        # and so on...
        for j in range(0, self.numTraining+1, 30) : # 7번 문제
